...
export class Graph {
    ...
    build() {
        Array.from(this.cache.values())
            ...
            .forEach((node, i) => node.ref['i'] = i)

        const fnRecycle = (node: Node, postRefs: Array<Node>) => {
            for (let i = 0; i < postRefs.length; i++) {
                const postRef = postRefs[i]
                if (postRef.prev.length === 1 && getChan(postRef) !== getChan(node))
                    this.signalChannel.release(getChan(postRef))
                if (postRef.prev.length === 2 && getChan(postRef) !== getChan(postRef.prev[0]))
                    this.signalChannel.release(getChan(postRef))
            }
        }
        ...
        const fnNegative = (node: Node) => {
            if (node.post.length === 0)
                setChan(node, this.signalChannel.apply())
            ...
            if (hasChan(node) && refs.length > 0) {
                 ...
                 */
                fnRecycle(node, node.post)

                if (!hasChan(refs[0]))
                    setChan(refs[0], ch)
                if (refs.length === 2 && refs[1].post.length === 1)
                    setChan(refs[1], this.signalChannel.apply())
            }
        }
